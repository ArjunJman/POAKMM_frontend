...
console.log("seats",seats);
  ...
  return (
    ...
    <div>
      <div className='bg-[#DCCA87] flex flex-col justify-center items-center h-[29rem]'>
        <button className='bg-[#0C0C0C] text-slate-200 rounded h-[3rem] w-[7rem] mt-3 mb-3 shadow-lg hover:shadow-2xl font-semibold' onClick={()=>setShowTickets(!showTickets)}>Book Tickets</button>
      { showTickets && 
      <div className='flex flex-col justify-center items-center'>
      <Seats ticketDetails={ticketDetails} id={id} />
      <TriggerButton type="button" onClick={handleOpen} sx={{backgroundColor:'#0C0C0C',color:'white',marginBottom:'10px'}}>
        Confirm Tickets!
      </TriggerButton>
      </div>}
      </div>
      ...
      >
        ...
        <ModalContent sx={{ width: 650,height: 480,padding:3,backgroundColor:'#0C0C0C',color:'white' }}>
          ...
          <hr />
            <h3 className='text-center font-semibold text-lg'>{ticketDetails['name']}</h3>
          <div className='grid grid-cols-2 gap-3'>
            <p className='font-semibold'>Category: {ticketDetails['cat']}</p>
            <p className='font-semibold justify-self-end'>Date: {ticketDetails['date']}</p>
            <p className='font-semibold '>Location: {ticketDetails['loc']}</p>
            <p className='font-semibold justify-self-end'>Venue: {ticketDetails['Venue']}</p>
          </div>
          <hr />
          <div className='flex justify-between'>
          <div>
          <p className='line-clamp-1 font-semibold'>Seat_No: {Object.entries(seats).map((seat)=>seat[1].id+' ')}</p>
          <p className='italic font-semibold'>{'( '+seats.length} {seats.length>1?'Tickets )':'Ticket )'}</p>
          </div>
          <p className='font-semibold'>RS.{seats.length * 100}</p>
          </div>
          <div className='flex justify-center'>
          <a href='/eventDetails' className='text-center pt-2 font-semibold w-[12rem] h-10 bg-[#DCCA87] rounded text-[#0C0C0C] no-underline '
